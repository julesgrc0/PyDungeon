...
import os, re, sys, random, math, time, threading
...
class Menu:
    ...
    def on_load(self):
        ...
        position = Vec(0, -100)
        wt =  (self.btp.get_render_size() - (self.btp.text_size("Map creator", fontsize) + margin*2))/2

        self.btn_mapcr.build("Map creator",wt + position, margin, fontsize)
        
        position.y += 100
        file = [f.replace('.dat','') for f in os.listdir(".") if f.endswith('.dat')]
        self.input_map.build(file[0] if len(file) > 0 else "", wt + position, margin, fontsize)
    ...
    def reset_input(self):
        self.input_map.build("", self.input_map.position, self.input_map.margin, self.input_map.button.text.fontsize)
    ...
    def on_draw_ui(self, dt: float) -> GameState:
        ...
        self.btp.draw_text(".dat",self.input_map.position + Vec(self.input_map.button.size.x + 20, self.input_map.button.size.y/2), self.input_map.button.text.fontsize, BLACK)

        self.btp.draw_rect(self.btn_mapcr.position-self.btn_mapcr.margin, Vec(5, 280), BLACK)
